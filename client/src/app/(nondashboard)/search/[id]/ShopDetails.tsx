import { ProductCategoryIcons, HighlightIcons  } from "@/lib/constants";
...
const ShopDetails = ({ vendorShopId }: ShopOverviewProps) => { 
  const {
    data: shop,
    isError,
    isLoading,
  ...
  return (
    <div className="mb-6">
      {/* Product Categories */}
      <div>
        <h2 className="text-xl font-semibold my-3">Products Offered</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {shop.productCategory.map((category) => {
            const catEnum = category as ProductCategoryEnum;
            const Icon = ProductCategoryIcons[category] || HelpCircle;
            return (
                <div key={category} className="flex flex-col items-center border rounded-xl py-8 px-4">
                {React.cloneElement(<Icon />, { className: "w-8 h-8 mb-2 text-gray-700" })}
                <span className="text-sm text-center text-gray-700">
                  {formatEnumString(catEnum)}
                </span>
              </div>
            );
          })}
      ...
      <div className="mt-12 mb-16">
        ...
        {shop.shopHighlight.map((highlight) => {
        const hlEnum = highlight as HighlightEnum;
        const Icon = HighlightIcons[hlEnum] || HelpCircle;
        return (
        ...
        <Tabs defaultValue="required-fees" className="mt-8">
          ...
          <TabsContent value="pets">
            <p className="font-semibold mt-5 mb-2">
              Pets are {shop.name ? "allowed" : "not allowed"}
            </p>
          </TabsContent>
          <TabsContent value="parking">
            <p className="font-semibold mt-5 mb-2">
              Parking is{" "}
              {shop.isParkingIncluded ? "included" : "not included"}
            </p>
          </TabsContent>
        </Tabs>
      </div>
